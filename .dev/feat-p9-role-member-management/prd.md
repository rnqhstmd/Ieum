...
## 구현 노트 (review 2회차 정합)
- **BR-6 호출 시점**: BR-6 본문의 "트랜잭션 커밋 후 admin 호출" 문구는 설계 §4 결정(`@Transactional` 내부에서 delete 직후 `disconnectUser` 직접 호출 — afterCommit 폐기, 단위 `verify` 가능)으로 **대체**된다. best-effort(예외 미전파)·localhost MVP 전제에서 commit 전/후 차이의 실질 위험은 낮으며, afterCommit 재이동은 P10+ 재검토 대상. (근거: trust-ledger.md review 2회차)
