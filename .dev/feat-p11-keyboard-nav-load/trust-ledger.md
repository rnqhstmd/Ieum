...
- **[GAP/MEDIUM→정보] getCaretOffset nodeType===3 가드 추가** — `Editor.tsx:64`. `el.contains` 가드는 **유지**되어 P6 cross-block offset 오염 방지 보존. nodeType 추가는 focus 직후/빈 블록 요소노드에서 fallback을 정확히 반환하는 강화(설계 의도). P6 커서 테스트(Editor.cursor.test.tsx)는 전부 fallback 경로라 회귀 무영향. 위험 없음.
