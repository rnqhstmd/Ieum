...
>
> **P5 후반 WS 인가 완료 (PR #15, feat/p5c-ws-auth — feat/p5b 위 적층)**: trust-relay userId + 멤버십 DB 인가(WS-AUTH-02/03). 백엔드 `/api/users/me`(실 userId 노출) + Node `MembershipStore`(pages⋈memberships) join 게이트(비멤버 `close(4003)`). **op는 인가 합류 page로만 영속/전파해 슬라이스1의 교차 room 영속화 인가 공백을 마감**, 연결 userId를 `crdt_ops.created_by_id`(Flyway V4) 태깅. 웹은 `/me` fetch 후 join에 userId trust-relay(relayClient `ready`로 첫 connect 레이스 제거). 검증: 단위+testcontainers(V1~V4) ws-relay 72 + web 141 + backend ✓. **후속**: 신원 위조 방지(trust-relay 한계), WS-AUTH-04(멤버 제거 강제종료, P7), 재접속 replay(P8).
...
| 항목 | 설명 | 상태 | Phase |
|------|------|------|-------|
| 재접속 replay | 50개 op 적용 후 Snapshot 생성 → 새 RGA에 Snapshot + 이후 op replay → 원본과 `toText()` 동일 | ✅ | P11 (PR #27) — 전량 op replay로 수렴 충족(Snapshot 생성은 후속). `pgOpStore.int` loadByPage round-trip(무손실 ASC) + ws-relay/web op-batch replay 수렴(AC-A1/A2/A5) |
...
| e2e (Playwright) | 브라우저 2개에서 동시 편집 후 양쪽 텍스트가 동일함을 확인 | ✅ | P11 (PR #27) — `apps/web/e2e/convergence.e2e.ts`(동시편집)·`restore.e2e.ts`(재접속) 작성. 로컬 수동 구동(자동 게이트 비포함, storageState 사전주입) |
