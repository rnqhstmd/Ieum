...
| US-PAGE-05 | 페이지 제목과 아이콘을 설정하고 싶다 | 제목 변경은 에디터 상단 인라인, 아이콘은 이모지 문자열로 저장 (직접 입력) | ✅ | P2 | `PageService.updatePage` 부분 갱신 + 사이드바 인라인 이름변경·이모지 직접입력 (PR #7). 에디터 상단 인라인 편집은 P3, 이모지 피커는 post-MVP |
