...
| `token` | `String` | UNIQUE | 수락 링크 토큰 — 앱 레이어에서 `crypto.randomBytes(32).toString('hex')`(256-bit)로 생성 |
...
| `seq` | `Int` | NOT NULL | 해당 siteId 내 단조 증가 시퀀스 번호 — CRDT 인과·멱등 보장용 |
| `serverSeq` | `BigInt` | `@default(autoincrement())` | 서버 수신 전역 단조 순번 — delta 동기화·스냅샷 워터마크용 |
...
**인덱스:** `(pageId, siteId, seq)`, `(pageId, serverSeq)`, `pageId`
...
| `version` | `Int` | NOT NULL | 스냅샷 생성 시점의 최대 serverSeq |
| `createdAt` | `DateTime` | DEFAULT now() | 생성 일시 |
...
wire 봉투 = `{ siteId, seq, opType, payload }` 이며, `payload`(이 §의 구조)는 opType에 따라 INSERT `{ id, originId, value }` / DELETE `{ targetId }`로 구성된다.
...
```
트리거: op 수 > 1,000개 OR 마지막 스냅샷 후 24시간 경과
처리:
  1. 최신 상태(모든 op replay 결과)를 Snapshot.state에 직렬화
  2. Snapshot.version = 현재 최대 serverSeq 값
...
2. SELECT * FROM CrdtOp WHERE pageId = ? AND serverSeq > {snapshot.version}
   ORDER BY serverSeq ASC -- 수렴은 인과버퍼가 보장
3. snapshot.state에 step 2의 op를 순서대로 적용
```

### 5.4 seq / siteId / serverSeq 의미

| 필드 | 의미 | 보장 사항 | 역할 |
|------|------|----------|------|
| `siteId` | 편집 클라이언트(세션) 식별자 | 세션 내 전역 유일 | CRDT 순서·tie-break |
| `seq` | 해당 siteId 내 op 발생 순서 | 단조 증가, siteId 범위 내 유일 | CRDT 인과·멱등 보장 |
| `(counter, siteId)` | op의 논리 식별자(벡터 클럭) | 전체 시스템 내 전역 유일 | CRDT 순서·tie-break |
| `serverSeq` | 서버 수신 전역 단조 순번 | DB autoincrement, 전역 유일 | delta 동기화·replay 워터마크 |

delta 동기화/replay 워터마크는 `serverSeq`(전역)를 기준으로 하고, CRDT 순서·tie-break는 `(counter, siteId)` 쌍으로 처리한다.
...
model Snapshot {
  ...
  pageId    String
  state     Json
  version   Int      // 스냅샷 시점의 최대 serverSeq
  createdAt DateTime @default(now())
