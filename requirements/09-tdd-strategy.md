...
- `rga.insert(doc, afterId, newId, value)` — 결과 배열 위치 확인
- `rga.delete(doc, targetId)` — tombstone 마킹, 렌더 텍스트 제외
...
**arbOp() 생성기 핵심**: 유효한 `{id: {counter, siteId}, value, originId, deleted}` 구조를 생성; Insert는 기존 doc 내 유효한 afterId 참조; Delete는 기존 문자 id 참조.
